...
作为最核心的AIAgent，`ChatClientAgent`构建了一个管道与LLM交互。为了让管道的输出更符合我们的需求，有输入增强（Input Enhancement）和输出增强（Output Enhancement）这两个主要的途径，前者通过通过改变输入让LLM返回更高质量的内容，后者则直接对LLM的输出进行加工处理。注册的`AIContextProvider`组成的管道位于`ChatClientAgent`管道的中间件部分（前后分别是`AIAgent`中间件管道和ChatClient管道），是专门为输入和输出增强设计的。RAG的本质是根据当前上下文检索相关内容来丰富LLM的输入，是输入增强的典型应用场景，在MAF中主要通过`TextSearchProvider`这个预定义的`AIContextProvider`实现。

## 1. 利用TextSearchProvider实现RAG

在介绍`TextSearchProvider`的设计和实现原理之前，我们先通过一个简单的例子来演示一下在MAF中如何使用`TextSearchProvider`实现RAG。首先来演示如下一个没有RAG的例子：我们根据`OpenAIClient`创建了一个`ChatClientAgent`，并直接调用它来回答一个问题：2026年斯诺克世界赛冠军是谁？
...
由于知识固化的原因，LLM无法直接回答这个问题。解决这个问题有两种途径: 

- 通过工具（比如`web-search`）获取最新信息；
- 通过RAG检索相关信息作为上下文来辅助LLM生成答案。

下面我们通过`TextSearchProvider`来实现RAG：我们在调用`AsAIAgent`时，传入一个`ChatClientAgentOptions`对象，并将一个`TextSearchProvider`实例添加到`AIContextProviders`中。`TextSearchProvider`需要我们提供一个委托来实现具体的检索逻辑，为此我们定义了`SearchAsync`方法返回吴宜泽夺冠的一段新闻稿。
...
RAG应该根据当前上下文来检索相关内容，生成的查询文本不仅仅只考虑最近的用户消息，有时还应该考虑最近或者全部对话历史。`TextSearchProvider`将作为检索的查询文本封装在`TextSearchProviderState`中，并作为Session的状态进行存储。如下面的代码片段所示，`TextSearchProviderState`是定义在`TextSearchProvider`中的一个内嵌类性，`RecentMessagesText`属性存储的就是作为检索的查询文本。`TextSearchProvider`利用一个`ProviderSessionState<TextSearchProviderState>`对象来extSearchProviderState进行基于Session状态的读写，它在Session状态字典中的Key可以通过`TextSearchProviderOptions`进行配置，默认为`TextSearchProvider`的类名。
...
重写的`ProvideMessagesAsync`会按照如下的逻辑执行：
